...
/*	printf("ORIGINAL-----------\n");
	for (int i = 0; i < n_top; i++) {
		printf("%s\t%f\n", cclist2[resultTable[i].id].ccode, resultTable[i].dist);
	}
	printf("ORIGINAL-----------\n"); */
	return(0);
}
...
int RecBase :: recognizeMultiEuclidean(FeatureVector& charvec, int gHint, CharCode *cclist){
	int i,j,ref_cnt,ref_id;
	cclist2 = cclist;
	recognizeEuclidean(charvec, gHint);
	// Group, considering classes
	double *counts = new double[n_cat];
	for ( i=0 ; i<n_cat ; i++ ){
		counts[i] = 0.0;
	}
	for ( i=0 ; i<n_top ; i++ ){
		if (resultTable[i].id == -1)
			continue;
...
//		printf("counts for %s = %f (group_size: %d)\n", cclist[resultTable[i].id].ccode, counts[ref_id], cclist[ref_id].group_size);
	...
	double *db_counts = new double[n_top];
	ref_cnt = 0;
	for ( i=0 ; i<n_cat ; i++ ){
		if (counts[i] > 0.0) {
			db_ids[ref_cnt] = i;
			db_counts[ref_cnt] = 1.0 / (1.0 + counts[i]);
			ref_cnt++;
		}
	}
/*	printf("REF_CNT: %d", ref_cnt);
	// Sort new
	for (int i = 0; i < n_top; i++) {
		if (resultTable[i].id == -1)
			continue;
		printf("%s\t%f\n", cclist[resultTable[i].id].ccode, resultTable[i].dist);
	} */
	for ( i=1 ; i<ref_cnt ; i++ ){
		j = i - 1;
		double value_count = db_counts[i];
		int value_id = db_ids[i];
		while ( j>=0 && db_counts[j]>value_count ){
			db_counts[j + 1] = db_counts[j];
			db_ids[j + 1] = db_ids[j];
			j--;
		}
		db_counts[j + 1] = value_count;
		db_ids[j + 1] = value_id;
	}
	// Fill resultTable again
	initResultTable();
	for ( i=0 ; i<ref_cnt ; i++ ){
		resultTable[i].id = db_ids[i];
		resultTable[i].dist = db_counts[i];
	}
/*	printf("----------\n");
	for (int i = 0; i < n_top; i++) {
		if (resultTable[i].id == -1)
