...
#ifdef DICDIR
static char	*nhocrlibdir0 = DICDIR;
#else
static char	*nhocrlibdir0 = "/opt/nhocr/share";
#endif
static char	*diccodes0 = "ascii+:jpn";

static char	*gramdportfile0 = "/var/run/gramd.port";
...
NHrec :: NHrec(){
	...
	nhocrdiccodes = diccodes0;
	gramdportfile = gramdportfile0;
	gramd_enabled = 0;
...
int NHrec :: rec_character(SIPImage *image, CharBox *cb, RecResultItem *resultTable){
	uchar	*p;
	int	cbw,cbh;
	int	n=0, cid;
	OCRPrep	OCRPrep;
	...
	Rec.recognizeMultiEuclidean(vec,0,cclist);
...
// FIXME with nice a proportional/fixed-width detection!
			double	cp;
			double	d;
