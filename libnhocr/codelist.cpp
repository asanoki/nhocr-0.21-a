...
int load_codelist(char *file, CharCode **cclist, int debug){
	...
	*cclist = cclist0;
	...
	for ( int i=0 ; i<n ; i++ ){
		cclist0[i].ref_id = -1;
		cclist0[i].group_size = 1;
		for ( int j=0 ; j<i ; j++ ){
			if ((strcmp(cclist0[i].ccode, cclist0[j].ccode) == 0) &&
			    (strcmp(cclist0[i].ccode_dic, cclist0[j].ccode_dic) == 0) &&
			    (cclist0[i].poshint == cclist0[j].poshint) &&
			    (cclist0[i].sizehint == cclist0[j].sizehint)) {
				printf("Joining %s to %s (%d vs. %d)\n", cclist0[i].ccode, cclist0[j].ccode,
					i, j);
				cclist0[i].ref_id = j;
				cclist0[j].group_size++;
				break;
			}
		}
	}
